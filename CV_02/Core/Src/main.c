...
#define BUTTON_PERIOD_S2 5
#define BUTTON_PERIOD_S1 5
/* USER CODE END PD */
...
void tlacitka(void)	{
	...
	if (Tick > off_time) {
	 ...
	 if (Tick > check_time_2 + BUTTON_PERIOD_S1) {
		check_time_2 = Tick;
		debounce <<= 1;
		if (LL_GPIO_IsInputPinSet(S1_GPIO_Port, S1_Pin) && 1) debounce |= 0x1;
		if (debounce == 0x7FFF) {
